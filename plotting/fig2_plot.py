...
def plot_all_with_contours_as_scatter(plot, data, labels, peaks, subpeaks, boxes, contours):
    # data[labels==0] = 0
    fig, ax = plt.subplots()
    # plt.title(plot)
    ...
    if contours is not None:
        for contour in contours:
            if contour is not None and contour.shape[0] != 0:
                ...
                #for contour_point in contour[0:len(contour)//10]:
                ...
                for cons in split_contours:
                    test.append(cons[::10])
                # print(contour.shape)
                # print(np.vstack(test).shape)
                for contour_point in np.vstack(test):
                #sorted = contour[np.lexsort((contour[:,1],contour[:,0]))]
                # for contour_point in sorted[::10]:
...
def plot_all_with_contours_as_image(plot, data, labels, peaks, subpeaks, boxes, contours):
    ...
    for peak in peaks:
        ax.scatter(peak[0] // downsampling_factor, peak[1], s=2, c='white', marker='o')

    if subpeaks is not None:
        for subpeak in subpeaks:
            if subpeak not in peaks:
                ax.scatter(subpeak[0] // downsampling_factor, subpeak[1], s=1, c='gray', marker='o')
    ...
    for box in boxes:
        ax.add_patch(Rectangle((box[2] // downsampling_factor, box[3]), (box[0]-box[2]) // downsampling_factor, (box[1]-box[3]), linewidth=1, edgecolor='red', facecolor='none'))
    ...
    if contours is not None:
        for contour in contours:
            if contour is not None and contour.shape[0] != 0:
                for contour_point in contour:
                    # print(contour_point)
                    test[contour_point[1], contour_point[0] // downsampling_factor] = 1
...
data_folder = "./fig2_data/"
# data_file = "atoms-2"
# data_file = "eeg-t1"
# data_file = "eeg-t1z"
# data_file = "rf-t12"

# for data_file in ["atoms-2"]:
for data_file in ["atoms-2", "eeg-t1", "eeg-t1z", "rf-t12"]:
    sbm_alg = '-sbm'
    ...
    plot_all_with_contours_as_image(data_file+sbm_alg+"-cds", sbm_data, sbm_labels, sbm_peaks, sbm_subpeaks, sbm_boxes, sbm_contours)
    ...
    #     plot_all_with_contours_as_image(data_file + pf_alg + "-cds", pf_data, pf_labels, pf_peaks, pf_subpeaks, pf_boxes, pf_contours)
    # except OSError:
    #     pf_data = np.copy(data)
    #     plot_all_with_contours_as_image(data_file + pf_alg + "-cds", pf_data, pf_labels, pf_peaks, None, pf_boxes, pf_contours)
    ...
    # plot_all_with_contours_as_image(data_file+oe_alg+"-cds", oe_data, oe_labels, oe_peaks, None, oe_boxes, None)
