...
def process_fft(data):
    data = np.array(data)
    train_fft_dataset = scipy.fft.fft(data, axis=1)
    # print(train_fft_dataset[:, 35:45])
    # print(data[:, :5])
    real = train_fft_dataset.real[:, 0:math.ceil(data.shape[1]/2)]
    imag = train_fft_dataset.imag[:, 0:math.floor(data.shape[1]/2)]
    data = np.hstack((real, imag))

    return data, train_fft_dataset.imag[:, math.ceil(data.shape[1]/2)]


def reverse_fft(data):
    data = np.array(data)
    real = data[:, 0:math.ceil(data.shape[1]/2)]
    imag = data[:, math.ceil(data.shape[1]/2):]
    imag = np.c_[imag, np.zeros(len(imag))]
    real_flip = np.flip(real, axis=1)
    imag_flip = np.flip(imag, axis=1)
    real = np.c_[real, real_flip[:, :-1]]
    imag = np.c_[imag, -1 * imag_flip[:, :-1]]

    complex = real + 1j*imag
    return scipy.fft.ifft(complex).real
